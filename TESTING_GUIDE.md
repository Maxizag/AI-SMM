...
### Вариант 1: Через простой bash-скрипт (Рекомендуется)
...
Этот скрипт:
- ✓ Показывает статистику до и после очистки
- ✓ Просит подтверждение перед удалением
- ✓ Удаляет все данные, кроме пользователей
- ✓ Работает через Docker (не требует локальных зависимостей)

### Вариант 2: Через Python-скрипт (требует установки зависимостей)

#### Если запускаете локально, сначала установите зависимости:
```bash
cd /path/to/AI-SMM
...
```bash
./clear_db.sh --user-id "UUID_ПОЛЬЗОВАТЕЛЯ"
```

### Вариант 3: Прямая очистка через psql
